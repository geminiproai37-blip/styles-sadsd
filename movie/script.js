...
// Get configuration from Inicio.html
const localMediaDataElement = document.getElementById("local-media-data");
const tmdbLogoLang = localMediaDataElement ? localMediaDataElement.dataset.tmdbLogoLang : "es"; // Default to 'es' if not found
export const categoriesParam = localMediaDataElement ? localMediaDataElement.dataset.categories : "popular,top_rated,upcoming"; // Default categories
...
export async function fetchMediaDetails(apiBaseUrl, type, id) {
  try {
    ...
    let episodeStills = [];
    const languagePriority = [tmdbLogoLang, "es-MX", "es-ES", "en"]; // Use configured language first
    ...
    try {
      const imagesUrl = `${apiBaseUrl}/${type}/${id}/images?api_key=${tmdbApiKey}&include_image_language=${tmdbLogoLang},es-MX,es-ES,en,ja,null`;
    ...
    if (mainImagesData.logos?.length > 0) {
      const priority = [tmdbLogoLang, "es-MX", "es-ES", "en", "ja", null]; // Use configured language first
...
export async function fetchMovieVideos(apiBaseUrl, movieId) {
  try {
    const url = `${apiBaseUrl}/movie/${movieId}/videos?api_key=${tmdbApiKey}&language=${tmdbLogoLang}`; // Use configured language
    const response = await fetch(url);
    if (!response.ok) {
...
export async function fetchCast(apiBaseUrl, type, id) {
  try {
    let url;
    if (type === "movie") {
      url = `${apiBaseUrl}/movie/${id}/credits?api_key=${tmdbApiKey}&language=${tmdbLogoLang}`; // Use configured language
    } else if (type === "tv") {
      url = `${apiBaseUrl}/tv/${id}/credits?api_key=${tmdbApiKey}&language=${tmdbLogoLang}`; // Use configured language
    } else {
