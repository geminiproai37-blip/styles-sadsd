...
// Gets the request data object { date, count } from localStorage.
function getRequestData() {
  const today = new Date().toISOString().split('T')[0];
  const storedData = JSON.parse(localStorage.getItem('requestData'));

  // If stored data is for today, return it. Otherwise, return a new object for today.
...
// Saves the request data object to localStorage.
...
export function openRequestModal() {
  ...
  const requestsLeft = MAX_REQUESTS_PER_DAY - requestData.count;
  ...
  if (requestCountSpan) {
    requestCountSpan.textContent = requestsLeft;
  }
...
export async function initBuscador(appMainContent) {
  ...
  if (submitRequestButton) {
    submitRequestButton.addEventListener("click", async () => {
      ...
      if (requestData.count >= MAX_REQUESTS_PER_DAY) {
        showNotification("Has alcanzado el límite de 10 solicitudes por día. Si quieres obtener más solicitudes, ve a Configuración y Solicitudes Points.", "error");
        return;
      }
      ...
      if (success) {
        requestData.count++; // Increment the count
        updateRequestCount(requestData); // Save the updated data
