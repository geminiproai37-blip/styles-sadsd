...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  // IMPORTANT: Be aware of the security implications of exposing these keys in client-side code.
  // Use the getGoogleApiKey function to retrieve the current key and handle fallbacks.
  let currentGoogleApiKey = getGoogleApiKey(); // Initialize with the first key
  ...
  const dom = {
    ...
    openQualityModalBtn: document.getElementById("open-quality-modal-btn"),
    ...
    openQualityModalBtnVertical: document.getElementById(
      "open-quality-modal-btn-vertical"
    ),
    ...
    popups: {
      ...
      qualitySelection: document.getElementById("quality-selection-modal"),
    },
  ...
  async function getGoogleDriveDirectLink(shareableUrl) {
    ...
    if (match && match[1]) {
      ...
      // Use the currentGoogleApiKey and handle potential failures
      let apiUrl = `https://www.googleapis.com/drive/v3/files/${fileId}?key=${currentGoogleApiKey}&alt=media`;
      ...
      try {
        ...
        if (!response.ok) {
          ...
          handleGoogleApiKeyFailure(); // Call the failure handler to try the next key
          currentGoogleApiKey = getGoogleApiKey(); // Get the next available key

          // If there are more keys, retry the fetch with the new key
          if (currentGoogleApiKey) {
            console.log("Retrying with next Google API key...");
            apiUrl = `https://www.googleapis.com/drive/v3/files/${fileId}?key=${currentGoogleApiKey}&alt=media`;
            return fetch(apiUrl); // Retry the fetch
          } else {
            // All keys have failed
            alert("Error al cargar el video de Google Drive. Todos los intentos con las claves API fallaron. Asegúrate de que el archivo sea público y las claves API sean válidas.");
            return null; // Indicate failure after all keys are exhausted
          }
        }
  ...
  function loadSource(server, startTime = 0) {
    ...
    // Hide quality button by default, show it only if HLS has multiple levels
    dom.openQualityModalBtn.classList.add("hidden");
    dom.openQualityModalBtnVertical.classList.add("hidden");
    ...
    const isPlayableVideoType = server.mp4 || server.hls || server.gdrive || server.mkv;

    if (isPlayableVideoType) {
      // Show video-related controls
      dom.video.classList.remove("hidden");
      dom.iframe.classList.add("hidden"); // Ensure iframe is hidden
      ...
      dom.poster.classList.remove("hidden"); // Show poster until video plays
    } else {
      // Hide video-related controls for non-video types (like iframes)
      dom.video.classList.add("hidden");
      dom.iframe.classList.remove("hidden"); // Show iframe
    ...
    if (server.mp4) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, function (event, data) {
        ...
        if (hls.levels.length > 1) {
          ...
          dom.openQualityModalBtn.classList.remove("hidden");
          dom.openQualityModalBtnVertical.classList.remove("hidden");
        }
      });
  ...
  const hideAllPopups = () => {
    ...
    dom.popups.qualitySelection.classList.add("hidden");
  };
  ...
  // Function to populate quality buttons in the modal
  function populateQualityButtons() {
    const qualityButtonsContainer = document.getElementById("quality-buttons-container");
    qualityButtonsContainer.innerHTML = ""; // Clear existing buttons
    let currentSelectedQualityButton = null;
    ...
    autoButton.classList.add("quality-button");
    if (currentLevel === -1) {
      autoButton.classList.add("selected");
      currentSelectedQualityButton = autoButton;
    }
    autoButton.dataset.level = -1;
    autoButton.innerHTML = `
        <div class="quality-info">
          <span class="quality-name-text">Automático</span>
        </div>
        <span class="material-icons checkmark-icon ${
          currentLevel === -1 ? "" : "hidden"
        }">check</span>
      `;
    autoButton.addEventListener("click", () => {
      hls.currentLevel = -1;
      updateQualityButtonSelection(autoButton);
      hideAllPopups();
    });
    ...
    levels.forEach((level, index) => {
      ...
      if (index === currentLevel) {
        button.classList.add("selected");
        currentSelectedQualityButton = button;
      }
      button.dataset.level = index;
      button.innerHTML = `
        <div class="quality-info">
          <span class="quality-name-text">${level.height}p</span>
        </div>
        <span class="material-icons checkmark-icon ${
          index === currentLevel ? "" : "hidden"
        }">check</span>
      `;
      button.addEventListener("click", () => {
        hls.currentLevel = index;
        updateQualityButtonSelection(button);
        hideAllPopups();
      });
      qualityButtonsContainer.appendChild(button);
    });

    function updateQualityButtonSelection(selectedButton) {
      if (currentSelectedQualityButton) {
        currentSelectedQualityButton.classList.remove("selected");
        const prevCheckmark =
          currentSelectedQualityButton.querySelector(".checkmark-icon");
        if (prevCheckmark) prevCheckmark.classList.add("hidden");
      }
      selectedButton.classList.add("selected");
      const newCheckmark = selectedButton.querySelector(".checkmark-icon");
      if (newCheckmark) newCheckmark.classList.remove("hidden");
      currentSelectedQualityButton = selectedButton;
    }
  }

  if (dom.openQualityModalBtn) {
    dom.openQualityModalBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      hideAllPopups();
      dom.popups.qualitySelection.classList.remove("hidden");
    });
  }

  if (dom.openQualityModalBtnVertical) {
    dom.openQualityModalBtnVertical.addEventListener("click", (e) => {
      e.stopPropagation();
      hideAllPopups();
      dom.popups.qualitySelection.classList.remove("hidden");
    });
